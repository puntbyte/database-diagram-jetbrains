...
import {NoteComponent} from "../components/note.ts";
    ...
    try {
      ...
      if (projectSettings) {
        ...
        if (projectSettings.zoom !== undefined || projectSettings.panX !== undefined) {
          ...
          // 2. GRID SIZE logic
          if (projectSettings.gridSize) {
            this.container.style.setProperty('--grid-size', `${projectSettings.gridSize}px`);
          } else {
            this.container.style.setProperty('--grid-size', '20px'); // Default
          }
        ...
        const showGrid = projectSettings.showGrid === true;
        if (showGrid) {
        ...
        if (notes && Array.isArray(notes)) {
          notes.forEach((note: DbNote) => {
            const noteEl = NoteComponent.create(note);
            this.wrapper.appendChild(noteEl);
          });
        }
      }
